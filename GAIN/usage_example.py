...
if __name__ == '__main__':
    ...
    df = pd.read_csv('./data/test_dataset_prob_dl_no_null.csv').drop(['Unnamed: 0'], axis=1).drop(['Output'], axis=1)
    print(df.head())
    data = df.sample(frac=1).reset_index(drop=True).values
    ...
    stopper = EarlyStopper(patience=2, min_delta=0.0000000000001)
    ...
    model.to(DEVICE)
    # model.train(n_epoches=1000, verbose=True, stopper=stopper)
    model.train(n_epoches=1000, verbose=True)
